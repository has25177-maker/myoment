...
# =========================================================
# 1. 커스텀 폰트 + 전역 스타일 (Ownglyph_PDH-Rg.woff2)
# =========================================================
def load_font_base64(font_path: str) -> str | None:
    try:
...
# =========================================================
# 5. 응급상황 AI – OpenStreetMap 기반 동물병원 검색
# =========================================================
OSM_USER_AGENT = "myoment-student-app/1.0"


def geocode_address(address: str):
    """Nominatim으로 주소 → 위도/경도"""
    try:
...
def search_animal_hospitals(lat: float, lon: float, radius_m: int = 2000):
    """
    Overpass API로 주변 동물병원(amenity=veterinary) 검색
    radius_m: 검색 반경 (미터)
    """
    query = f"""
    [out:json];
    ...
    try:
        ...
        for el in elements:
            ...
            map_rows.append({"lat": lat_el, "lon": lon_el})
            table_rows.append({"이름": name, "주소": addr, "전화번호": phone})

        if not map_rows:
            return None, None

        return pd.DataFrame(map_rows), pd.DataFrame(table_rows)
    except Exception:
        return None, None
...
def page_ai_emergency():
    ...
    region = st.text_input("주소 입력 (예: 서울 강남구, 부산 해운대구 등)")
    if st.button("검색"):
        if not region.strip():
            st.warning("먼저 주소를 입력해 주세요.")
        else:
            geo = geocode_address(region.strip())
            if not geo:
                st.warning("주소를 찾을 수 없습니다.")
            else:
                lat, lon = geo
                map_df, table_df = search_animal_hospitals(lat, lon)
                if map_df is None:
                    st.info("주변 반경에서 동물병원을 찾지 못했습니다.")
                else:
