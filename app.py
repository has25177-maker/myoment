...
# 🔑 카카오 REST API 키를 여기에 입력해야 병원 검색이 작동합니다.
KAKAO_REST_API_KEY = "여기에_카카오_REST_API_키_입력"  # ← 직접 수정하세요


# ======================================
# 1. 폰트(BakDahyun.woff2) + 전역 스타일
# ======================================
font_css = ""
try:
    with open("BakDahyun.woff2", "rb") as f:
        font_bytes = f.read()
...
# ======================================
# 5. 응급상황 AI (+ 카카오맵 병원 검색)
# ======================================
def search_animal_hospitals_kakao(keyword_region: str):
    """카카오 API로 주소→좌표→주변 동물병원 검색"""
    if not KAKAO_REST_API_KEY or "여기에_카카오" in KAKAO_REST_API_KEY:
        st.error("카카오 REST API 키를 먼저 코드 상단 KAKAO_REST_API_KEY에 입력해 주세요.")
        return None, None
    ...
    if addr_res.status_code != 200:
        st.error("주소를 불러오는 중 오류가 발생했습니다.")
        return None, None

    addr_docs = addr_res.json().get("documents", [])
    if not addr_docs:
        st.warning("해당 주소를 찾을 수 없습니다. 예: '서울 강남구', '부산 해운대구' 등으로 입력해 주세요.")
        return None, None

    x = float(addr_docs[0]["x"])  # 경도
    y = float(addr_docs[0]["y"])  # 위도
    ...
    if res.status_code != 200:
        st.error("동물병원을 검색하는 중 오류가 발생했습니다.")
        return None, None

    docs = res.json().get("documents", [])
    if not docs:
        st.info("주변 5km 이내 동물병원이 검색되지 않았습니다.")
        return None, None
    ...
    for d in docs:
        ...
        table_data.append({"이름": name, "주소": addr, "전화번호": phone})
...
def page_ai_emergency():
    ...
    region = st.text_input("주소 또는 동네를 입력해 주세요 (예: 서울 강남구, 부산 해운대구 등).", key="region_input")
    search_clicked = st.button("동물병원 검색")

    if search_clicked:
        if not region.strip():
            st.warning("먼저 주소나 동네를 입력해 주세요.")
        else:
