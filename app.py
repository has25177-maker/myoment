...
if "food_db" not in st.session_state:
    # 음식 사전 간단 DB (예시, 사람이 자주 먹는 음식 위주)
    st.session_state.food_db = {
        ...
        "소고기": {
            "가능": "기름기 적은 부위를 잘 익혀 소량 가능",
            "주의": "양념/마늘/양파/소금 절대 금지",
        },
        "돼지고기": {
            "가능": "잘 익혀서 소량 가능하나, 다른 단백질에 비해 부담될 수 있음",
            "주의": "기름기, 양념, 튀김류는 소화 부담이 큼",
        },
        "참치캔": {
        ...
        "연어": {
            "가능": "익힌 연어를 소량 간식으로 가능",
            "주의": "생연어, 훈제연어, 양념된 연어는 피하기",
        },
        "멸치": {
            "가능": "염분이 적은 멸치를 소량 간식 정도로 가능",
            "주의": "국멸치/멸치볶음은 염분이 높을 수 있음",
        },
        "우유": {
        ...
        "요구르트": {
            "가능": "설탕·향료가 없는 무가당 요거트를 아주 소량만",
            "주의": "설탕, 인공감미료(자일리톨) 포함 제품은 피하기",
        },
        "치즈": {
            "가능": "소량의 무염 치즈만 가능",
            "주의": "염분과 유당이 많아 자주 주면 안 좋음",
        },
        "빵": {
            "가능": "맛보는 수준의 한 두 조각 정도만 가능",
            "주의": "버터, 설탕, 초콜릿, 크림이 들어간 빵은 금지",
        },
        "초콜릿": {
            "가능": "불가",
        ...
        "건포도": {
            "가능": "불가",
            "주의": "포도와 동일하게 신장 손상 위험, 절대 금지",
        },
        "아몬드": {
            "가능": "급여 비추천",
            "주의": "지방이 많고, 향미/소금 처리된 제품은 특히 위험",
        },
        "호두": {
            "가능": "급여 비추천",
            "주의": "지방이 많고 질식 위험, 일부 견과류는 독성 가능성",
        },
        "사과": {
        ...
        "수박": {
            "가능": "씨와 껍질을 제거한 과육을 소량 간식으로 가능",
            "주의": "과당이 있어 너무 자주/많이 주는 것은 좋지 않음",
        },
        "고구마": {
            "가능": "잘 익힌 상태로 소량 가능",
            "주의": "당분이 있어 비만/당뇨가 있는 경우 주의",
        },
        "감자": {
            "가능": "잘 익힌 감자만, 소량 간식으로 가능",
            "주의": "생감자/껍질/싹은 독성, 절대 금지",
        },
        "브로콜리": {
            "가능": "삶아서 소량 가능",
            "주의": "날로 많이 먹이면 가스를 유발할 수 있음",
        },
        "커피": {
            "가능": "불가",
            "주의": "카페인 독성, 아주 소량도 위험",
...
def page_guide():
    ...
    with tabs[0]:
        basic_lines = [
            ...
            "화장실, 물, 밥그릇은 서로 너무 가깝지 않게 배치하는 것이 좋아요.",
    ...
    with tabs[1]:
        ...
        lines = [
            ...
            "약 먹이기 직후 간단한 간식을 주어 긍정적인 경험으로 연결해 주세요.",
        ]
        for l in lines:
            st.write("- " + l)
    ...
    with tabs[5]:
        ...
        lines = [
            ...
            "높은 곳 대신 낮은 곳에도 쉴 수 있는 공간을 마련하고, 점프를 줄일 수 있도록 발판을 두는 것도 도움이 됩니다.",
        ]
        for l in lines:
            st.write("- " + l)
    ...
    with tabs[7]:
        ...
        lines = [
            ...
            "급격한 체중 감소는 지방간 등 심각한 문제를 유발할 수 있으니 천천히 줄이는 것이 중요합니다.",
        ]
        for l in lines:
            st.write("- " + l)
    ...
    with tabs[8]:
        ...
        lines = [
            ...
            "구강 문제: 침 흘림, 입 냄새, 딱딱한 사료를 씹기 어려워함, 한쪽으로만 씹음.",
            "호흡기 문제: 기침, 헐떡임, 쉬는 상태에서 호흡수가 빠름.",
        ]
        for l in lines:
            st.write("- " + l)
...
# 페이지 4. 응급상황 AI (+ 지도 예시)
# -----------------------------
def guess_region_coords(region_text: str):
    """아주 간단한 문자열 매칭으로 좌표 추정 (데모용)"""
    region_text = region_text.strip()
    if not region_text:
        return 37.5665, 126.9780, "서울 시청 근처 (기본값)"

    lower = region_text.lower()
    mapping = [
        ("강남", 37.4979, 127.0276, "서울 강남역 근처"),
        ("마포", 37.5527, 126.9378, "서울 마포구 합정/홍대 근처"),
        ("송파", 37.5145, 127.1050, "서울 송파구 잠실 근처"),
        ("인천", 37.4563, 126.7052, "인천 시청 근처"),
        ("부산", 35.1796, 129.0756, "부산 시청 근처"),
        ("대구", 35.8714, 128.6014, "대구 시청 근처"),
        ("대전", 36.3504, 127.3845, "대전 시청 근처"),
        ("광주", 35.1595, 126.8526, "광주 시청 근처"),
    ]

    for key, lat, lon, label in mapping:
        if key in region_text or key in lower:
...
def page_ai_emergency():
    ...
    region = st.text_input("거주 동네(예: 강남구, 마포구, 부산 등) - 병원/대피소 안내 예시용", "")

    # 지도 예시
    if region.strip():
        lat, lon, label = guess_region_coords(region)
        st.markdown(f"#### 🗺️ {label} 기준으로 표시한 지도 (예시)")
        map_df = pd.DataFrame(
            {
        ...
        st.caption("※ 실제 병원/대피소 위치는 연동된 지도 API를 통해 더 정확히 표시할 수 있습니다.")

        st.markdown("##### 예시 동물병원/대피소 리스트")
        st.table(
            pd.DataFrame(
                {
                    "이름": ["24시 동물의료센터", "OO동물병원", "OO구 임시 대피소"],
                    "설명": ["응급 가능", "일반 진료", "재난 시 대피 장소(예시)"],
                }
            )
        )
    ...
    if mode == "건강 관련 응급":
        ...
        st.subheader("❤️ 반려동물 심폐소생술 (기본 개념 안내)")
        st.warning(
        ...
        st.info("실제 상황에서는 24시 동물병원과 즉시 연락하고, 안내에 따라 행동하는 것이 가장 안전합니다.")
        ...
        st.write("- 화재 시, 집 안 곳곳을 오래 찾기보다 가장 가까운 고양이부터 이동장에 넣어 빠르게 대피합니다.")
...
def page_food_dict():
    ...
    st.subheader("🔎 음식 검색")
    query = st.text_input("사람 음식 이름을 입력하세요 (예: 양파, 사과, 초콜릿, 고구마 등)")
    ...
    with st.expander("📃 현재 등록된 음식 목록 보기"):
        df_food = pd.DataFrame(
            [
                {"음식": name, "급여 가능 여부/조건": info["가능"], "주의사항": info["주의"]}
